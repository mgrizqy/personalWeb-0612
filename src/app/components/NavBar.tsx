    ...
    <>
      <nav className="fixed w-full h-[85px] bg-[#111015]/80 backdrop-blur-sm flex items-center justify-center z-50 border-b border-white/5">
        <div className="flex items-center justify-between w-full max-w-[1700px] px-4 md:px-8">
          <a
            href="#home"
            className="h-[40px] px-3 rounded-lg flex items-center justify-center 
                    border-2 border-dashed border-[#adff30]/20 text-[#adff30] 
                    hover:bg-[#adff30] hover:text-black hover:scale-110  
                    transition-all duration-300 cursor-pointer group"
          >
            <p className="tracking-wide font-extrabold drop-shadow-[0_0_8px_rgba(173,255,48,0.17)]">
              {"<e/>"}
            </p>
          </a>
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <button
              ...
              className="relative h-[44px] w-[116px] bg-[#e7e7e7] rounded-[5px] text-black flex items-center justify-center font-bold hover:scale-105 transition-transform"
            >
              <strong>Hire Me</strong>
              <span
                className="w-[10px] h-[10px] bg-[#adff30] rounded-full absolute z-10"
                style={{ top: "-4px", right: "-4px" }}
              ></span>
              <span
                className="w-[10px] h-[10px] bg-[#adff30] rounded-full absolute"
                style={{
                  top: "-4px",
                  right: "-4px",
                  animation:
                    "scale 2s ease-out infinite, fade-out 2s ease-out infinite",
                }}
              ></span>
            </button>
          </div>
...
`}
        >
          <ul className="flex flex-col items-center justify-center h-full gap-8 text-lg pt-[85px] pb-8">
            {" "}
            {/* Smaller text/gap */}
            {navLinks.map((link) => (
              <NavLink key={link.href} {...link} />
